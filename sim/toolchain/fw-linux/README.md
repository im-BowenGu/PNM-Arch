# fw-linux — Linux tinyconfig + bare-metal init for router_sbc

Produces a minimal bootable Linux image for the `router_sbc` chip family.
...
- `riscv64-unknown-linux-musl-gcc` cross compiler (available in `shell.nix`)
...
| DRAM | `0x8000_0000` | 512 MB | LPDDR5, heap + data |
...
The NVMe driver itself is `sim/fw/pnm_nvme.c` (shared with MCU/SoC
