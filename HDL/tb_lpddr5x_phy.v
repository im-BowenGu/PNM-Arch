...
// =============================================================================
// tb_lpddr5x_phy — Self-checking testbench for LPDDR5X PHY controller
// =============================================================================

module tb_lpddr5x_phy;
    ...
    reg [31:0] rdata;
    initial begin
        $dumpfile("lpddr5x_phy.vcd");
        $dumpvars(0, tb_lpddr5x_phy);
        errors = 0;
        rst_n = 0;
        ...
        // Test 1: Write/read basic
        $display("Test 1: Write/read basic");
        write_word(32'h00000000, 32'hCAFEBABE);
        ...
        write_word(32'h00000010, 32'hAAAA_BBBB);
        write_word(32'h00000020, 32'hCCCC_DDDD);
        read_word(32'h00000010, rdata);
        if (rdata !== 32'hAAAA_BBBB) begin
            $display("  FAIL addr 0x10: got %h", rdata); errors = errors + 1;
        end else $display("  OK: %h", rdata);
        read_word(32'h00000020, rdata);
        if (rdata !== 32'hCCCC_DDDD) begin
        ...
        if (errors == 0)
            $display("*** LPDDR5X PHY TEST PASSED ***");
        else begin
            $display("*** LPDDR5X PHY TEST FAILED (%0d errors) ***", errors);
            $finish(1);
        end
        $finish;
    end
