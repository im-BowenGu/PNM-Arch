...
module tb_fp16_mac_array;
    ...
    initial begin
        ...
        // Track whether result_valid fired (regression: it used to never assert
        // because the capture sampled fma_valid_out[bottom][0] after it pulsed).
        result_seen = 0;
        repeat (60) begin
            @(posedge clk); #1;
            if (result_valid)
        ...
        if (!result_seen) begin
            ...
            if (result_out[15:0] !== FP16_1) begin
                $display("[TB] FAIL: result_out[0] = %h, expected %h", result_out[15:0], FP16_1);
                errors = errors + 1;
            end
        end
