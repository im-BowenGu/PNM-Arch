...
//   [6]    AXIS  : 0 = X, 1 = Y
//   [5]    SIGN  : 0 = +, 1 = -
//   [4:0]  DIST  : hop distance from the lxy_repeater
...
module pe_tile_stub #(
    ...
    // stripped at the lxy_repeater and the DMA stream carries no direction.
    ...
    // -- INT4 MAC compute path (CU_TYPE=2) ----------------------------------
    // Uses weight_dequant + int8_mac for W8A8 inference.
    // Activation bytes (INT8) are multiplied by INT8_WEIGHT and accumulated
    // into a 32-bit running sum, then serialized back to the byte stream.
    wire [15:0] deq_bf16_w;
    wire        deq_valid_w;
    ...
    weight_dequant u_deq (
        ...
        .rst_n     (rst_n),
        .mode      (1'b1),         // INT8 mode
        .weight_in (INT8_WEIGHT),
        .nibble_sel(1'b0),
        .valid_in  (deliver && in_payload && (CU_TYPE == 2)),
        .bf16_out  (deq_bf16_w),
        .valid_out (deq_valid_w)
    );

    int8_mac u_mac (
        .clk       (clk),
        .rst_n     (rst_n),
        .a         (s1_data),       // activation (signed INT8)
        .b         (INT8_WEIGHT),   // weight (signed INT8)
        .c         (int4_acc),      // accumulator
        .valid_in  (deliver && in_payload && (CU_TYPE == 2)),
        .result    (mac_result_w),
    ...
    wire [7:0] int4_out_byte = int4_out_sr[7:0];
    ...
    // entry says this node sits DIST hops from the lxy_repeater along AXIS
