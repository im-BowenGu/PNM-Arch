...
// =============================================================================
// tb_lpddr5_phy — Self-checking testbench for LPDDR5 PHY controller
// =============================================================================

module tb_lpddr5_phy;
    ...
    reg [31:0] rdata;
    initial begin
        $dumpfile("lpddr5_phy.vcd");
        $dumpvars(0, tb_lpddr5_phy);
        errors = 0;
        rst_n = 0;
        bus_addr = 0; bus_wdata = 0; bus_we = 0;
        bus_be = 0; bus_valid = 0;
        doorbell_trig = 0;
        ...
        if (rdata !== 32'hDEADBEEF) begin
            $display("  FAIL: got %h, expected DEADBEEF", rdata);
            errors = errors + 1;
        end else
            $display("  OK: %h", rdata);
        ...
        write_word(32'h00000010, 32'h11111111);
        write_word(32'h00000020, 32'h22222222);
        read_word(32'h00000010, rdata);
        if (rdata !== 32'h11111111) begin
            $display("  FAIL addr 0x10: got %h", rdata);
            errors = errors + 1;
        end else
            $display("  OK: %h", rdata);
        read_word(32'h00000020, rdata);
        if (rdata !== 32'h22222222) begin
            $display("  FAIL addr 0x20: got %h", rdata);
            errors = errors + 1;
        end else
            $display("  OK: %h", rdata);
        ...
        if (dram_read_count < 2 || dram_write_count < 2) begin
            ...
            $display("  OK: reads=%0d writes=%0d refreshes=%0d",
                     dram_read_count, dram_write_count, dram_refresh_count);

        // Test 4: Sideband
        $display("Test 4: Sideband");
        if (!topology_rdy || node_err) begin
            $display("  FAIL: topology_rdy=%b node_err=%b", topology_rdy, node_err);
            errors = errors + 1;
        end else
            $display("  OK");

        if (errors == 0)
            $display("*** LPDDR5 PHY TEST PASSED ***");
        else begin
            $display("*** LPDDR5 PHY TEST FAILED (%0d errors) ***", errors);
            $finish(1);
        end
        $finish;
    end
