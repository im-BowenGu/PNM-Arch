...
#ifdef PNM_MCU
/* MCU targets: minimal limits to fit in 4-8 KB SRAM */
#define PNM_MAX_LAYERS      2
#define PNM_MAX_NODES       16
#define PNM_MAX_NODES_TOTAL 32
#define PNM_MAX_EXPERTS     16
#define PNM_MAX_TOPK        4
...
typedef struct {
    ...
    int     *entry_seq;         /* parallel: seqPos stored per slot (optional) */
    int      depth;
    int      entry_bytes;
    int      write_ptr;
    int      read_ptr;
...
typedef struct {
    ...
    int             moe_count;
    int             top_k_experts; /* active experts per token (0 = default) */
