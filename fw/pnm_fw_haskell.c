...
static uint64_t haskell_fnv1a_64(const uint8_t *token, int token_len) {
...
static int haskell_select_topk(const uint8_t *token, int token_len, int ml,
                               const int *experts, int n, int topk, int *topk_experts) {
    if (n <= 0) return 0;
    if (topk > n) topk = n;
    uint64_t h = haskell_fnv1a_64(token, token_len);
                              ...
                              int max_records, int *num_records)
{
    ...
    for (int ml = 0; ml < model_layers && idx < max_records; ml++) {
        ...
        /* Step 3: MoE gating — FP64 FMA. Route to the top-k experts by a
        ...
        int nsel = haskell_select_topk(token, token_len, ml, cand, cand_n, topk, sel);
                              ...
                              int num_records)
{
    ...
    for (int i = 0; i < num_records; i++) {
        ...
        int elems = (flit_bytes - 7) / HASKELL_DTYPE_BYTES; /* minus header+CRC */
        if (elems < 0) elems = 0;
